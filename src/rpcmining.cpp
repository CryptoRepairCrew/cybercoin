...
Value getstakesubsidy(const Array& params, bool fHelp)
{
    ...
    return (uint64_t)GetProofOfStakeReward(nCoinAge, 0, GetLastBlockIndex(pindexBest, true));
}
...
Value getstakinginfo(const Array& params, bool fHelp)
{
    ...
    Object obj,interest;
    std::vector<int64_t> vPOS(6);
    std::vector<int64_t> slabs(5);
    vPOS=vPOSslabs(pindexBest->nHeight);
    slabs=GetPosSlabs(pindexBest->nMoneySupply);
    std::string slabStr;
    std::stringstream sstm;
    sstm << static_cast<double>(vPOS[0])/100 << "% Min block 0";
    slabStr = sstm.str();
    interest.push_back(Pair("slab 1", slabStr));
    sstm.str("");
    sstm << static_cast<double>(vPOS[1])/100 << "% Min block " << slabs[0]/100000000;
    slabStr = sstm.str();
    interest.push_back(Pair("slab 2", slabStr));
    sstm.str("");
    sstm << static_cast<double>(vPOS[2])/100 << "% Min block " << slabs[1]/100000000;
    slabStr = sstm.str();
    interest.push_back(Pair("slab 3", slabStr));
    sstm.str("");
    sstm << static_cast<double>(vPOS[3])/100 << "% Min block " << slabs[2]/100000000;
    slabStr = sstm.str();
    interest.push_back(Pair("slab 4", slabStr));
    sstm.str("");
    sstm << static_cast<double>(vPOS[4])/100 << "% Min block " << slabs[3]/100000000;
    slabStr = sstm.str();
    interest.push_back(Pair("slab 5", slabStr));
    sstm.str("");
    sstm << static_cast<double>(vPOS[5])/100 << "% Min block " << slabs[4]/100000000;
    slabStr = sstm.str();
    interest.push_back(Pair("slab 6", slabStr));
    sstm.str("");
    ...
    obj.push_back(Pair("interest", interest));
    return obj;
}
...
Value checkkernel(const Array& params, bool fHelp)
{
    ...
    if (vNodes.empty())
        throw JSONRPCError(-9, "CyberCoin is not connected!");

    if (IsInitialBlockDownload())
        throw JSONRPCError(-10, "CyberCoin is downloading blocks...");
...
Value getwork(const Array& params, bool fHelp)
{
    ...
    if (vNodes.empty())
        throw JSONRPCError(RPC_CLIENT_NOT_CONNECTED, "CyberCoin is not connected!");

    if (IsInitialBlockDownload())
        throw JSONRPCError(RPC_CLIENT_IN_INITIAL_DOWNLOAD, "CyberCoin is downloading blocks...");
