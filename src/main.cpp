...
const string strMessageMagic = "CyberCoin Signed Message:\n";
...
int64_t GetProofOfWorkReward(int64_t nFees, unsigned int nHeight)
{
    ...
    if (nHeight > 3463){
    nSubsidy = 2000 * COIN;
    }else if (nHeight > 2788){
    nSubsidy = 4000 * COIN;
    }else if (nHeight > 2113){
    nSubsidy = 10000 * COIN;
    }else if (nHeight > 1438){
    nSubsidy = 20000 * COIN;
    }else if (nHeight > 763){
    nSubsidy = 37000 * COIN;
    }else if (nHeight > 88){
...
std::vector<int64_t> GetPosSlabs(double supply) {
    std::vector<int64_t> slabs(5);
    uint64_t base=static_cast<int64_t>(supply);
    slabs[0]=static_cast<int64_t>(base*0.005);
    slabs[1]=static_cast<int64_t>(base*0.01);
    slabs[2]=static_cast<int64_t>(base*0.05);
    slabs[3]=static_cast<int64_t>(base*0.1);
    slabs[4]=static_cast<int64_t>(base*0.2);
    return slabs;
}

int64_t GetInterestRate(const CBlockIndex* pindexLast,int64_t nCredit,int nHeight)
{
double supply;
std::vector<int64_t> slabs(5);
std::vector<int64_t> vPOS(6);

supply = pindexLast->nMoneySupply;
slabs=GetPosSlabs(supply);
vPOS=vPOSslabs(nHeight);
...
return static_cast<int64_t>(rate);
}
// miner's coin stake reward based on coin age spent (coin-days)
int64_t GetProofOfStakeReward(int64_t nCredit, int64_t nFees,const CBlockIndex* pindexLast)
{

    int64_t nSubsidy = static_cast<int64_t>(nCredit * GetInterestRate(pindexLast,nCredit,pindexLast->nHeight)/10000) ;

    //int64_t nSubsidy = nCoinAge * COIN_YEAR_REWARD * 33 / (365 * 33 + 8);

    LogPrint("creation", "GetProofOfStakeReward(): create=%s nCoinAge=%d\n", FormatMoney(nSubsidy), nCredit);
...
bool CBlock::AcceptBlock()
{
    ...
    if (IsProofOfStake() && nHeight < LAST_POW_BLOCK-POS_START_BUFFER)
            return DoS(100, error("AcceptBlock() : reject proof-of-stake at height < %d", LAST_POW_BLOCK-POS_START_BUFFER));
...
{
    RenameThread("cybercoin-loadblk");
